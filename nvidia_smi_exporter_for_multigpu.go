...
func metrics(response http.ResponseWriter, request *http.Request) {
    ...
    cmd := "nvidia-smi | grep 'python' | awk '{ print $2 \" \" $3 }'"
	...
	temp := fmt.Sprintf("%s",row[0])
	// print(temp)
	tmpwords := strings.Fields(temp)

//	fmt.Println(tmpwords[0], tmpwords[1])
        name := fmt.Sprintf("%s[%s]", result_gpu_name, tmpwords[0])
        result = fmt.Sprintf(
            "%s%s{gpu=\"%s\"} %s\n", result,
            metricList[6], name, tmpwords[1])
    }
    len, err := strconv.Atoi(result_nth)
    for (fl<=len) {
        name := fmt.Sprintf("%s[%s]", result_gpu_name, strconv.Itoa(fl))
        result = fmt.Sprintf(
            "%s%s{gpu=\"%s\"} %s\n", result,
            metricList[6], name, "0")
	fl+=1
